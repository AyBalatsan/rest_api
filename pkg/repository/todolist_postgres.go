...
func (r *TodoListPostgres) Create(userId int, list todo.TodoList) (int, error) {
	...
	row := tx.QueryRow(createListQuery, list.Title, list.Descriprtion)
	if err := row.Scan(&idRowList); err != nil {
		tx.Rollback()
		return 0, err
	}

	createUsersListQuery := fmt.Sprintf("INSERT INTO %s (user_id, list_id) VALUES ($1, $2)", usersListsTable)
	_, err = tx.Exec(createUsersListQuery, userId, idRowList)
	if err != nil {
		tx.Rollback()
		return 0, err
	}
	return idRowList, tx.Commit()
}
...
func (r *TodoListPostgres) GetById(idUser, id int) (todo.TodoList, error) {
	...
	err := r.db.Get(&list, query, idUser, id)
	return list, err
}
